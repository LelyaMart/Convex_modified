...
class TestVoid:
    ...
    def test_inside_perimeter(self):
        assert self.f.inside_perimeter() == 0.0
...
class TestSegment:
    ...
    # Обе точки лежат в первом квадранте
    def test_inside_perimeter1(self):
        assert self.f.inside_perimeter() == approx(2)

    # Только одна лежит в первом квадранте
    def test_inside_perimeter2(self):
        assert Segment(R2Point(1, 1), R2Point(1, -1)).inside_perimeter() \
               == approx(2.0)

    # Нет точек в первом квадранте
    def test_inside_perimeter3(self):
        assert Segment(R2Point(-1, 1), R2Point(-1, -1)).inside_perimeter() \
               == approx(0)
...
class TestPolygon:
    ...
    def test_inside_perimeter1(self):
        assert self.f.inside_perimeter() == approx(2.0 + sqrt(2.0))

    def test_inside_perimeter2(self):
        assert self.f.add(R2Point(1.0, 1.0)).inside_perimeter() == approx(4.0)

    def test_inside_perimetr3(self):
        assert Polygon(R2Point(1, 1), R2Point(0, 0),
                       R2Point(1, -1)).inside_perimeter() \
               == approx(1 + sqrt(2))

    def test_inside_perimetr4(self):
        assert Polygon(R2Point(0, -2), R2Point(3, -2),
                       R2Point(2, 0)).inside_perimeter() == approx(0)

    def test_add1(self):
        f = Polygon(R2Point(3, 0), R2Point(0, 3), R2Point(0, 0))
        f.add(R2Point(4, 0))
        assert f.inside_perimeter() == f.perimeter() == approx(12)
    ...
    def test_add3(self):
        f = Polygon(R2Point(3, 0), R2Point(0, -4), R2Point(0, 0))
        assert f.inside_perimeter() == approx(3)
        f.add(R2Point(3, -4))
        assert f.inside_perimeter() == approx(3)

    def test_add4(self):
        f = Polygon(R2Point(-3, 0), R2Point(0, -4), R2Point(0, 0))
        assert f.inside_perimeter() == approx(0)
        f.add(R2Point(-3, -4))
        assert f.inside_perimeter() == approx(0)

    def test_add5(self):
        f = Polygon(R2Point(5, 5), R2Point(-5, -5), R2Point(5, -5))
        f.add(R2Point(-5, 5))
        assert f.inside_perimeter() == approx(10)
