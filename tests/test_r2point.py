...
class TestR2Point:
    ...
    def test_dist_inside_quadrant1(self):
        a, b = R2Point(0.0, 0.0), R2Point(1.0, 1.0)
        assert a.dist_inside_quadrant(b) == approx(sqrt(2))

    def test_dist_inside_quadrant2(self):
        a, b = R2Point(1.0, 0.0), R2Point(1.0, 1.0)
        assert a.dist_inside_quadrant(b) == approx(1)

    def test_dist_inside_quadrant3(self):
        a, b = R2Point(0.0, 1.0), R2Point(1.0, 1.0)
        assert a.dist_inside_quadrant(b) == approx(1.0)

    def test_dist_inside_quadrant4(self):
        a, b = R2Point(2.0, 2.0), R2Point(1.0, 1.0)
        assert a.dist_inside_quadrant(b) == approx(sqrt(2))

    def test_dist_inside_quadrant5(self):
        a, b = R2Point(-1, 1), R2Point(1.0, 1.0)
        assert a.dist_inside_quadrant(b) == approx(1)

    def test_dist_inside_quadrant6(self):
        a, b = R2Point(0.0, 1.0), R2Point(0, -1)
        assert a.dist_inside_quadrant(b) == approx(1)

    def test_dist_inside_quadrant7(self):
        a, b = R2Point(-1.0, -1.0), R2Point(-2, -1)
        assert a.dist_inside_quadrant(b) == approx(0)

    def test_dist_inside_quadrant8(self):
        a, b = R2Point(1.0, -1.0), R2Point(0, 0)
        assert a.dist_inside_quadrant(b) == approx(0)
